...
# Function to setup multipath routing with 65-35 distribution
setup_multipath() {
...
while true; do
    ...
    if [ $isp1_net_status -eq 0 ] && [ $isp2_net_status -eq 0 ]; then
        if [ ! -f /tmp/load_balance_active ]; then
            ...
            log_message "Both ISPs online - Load balancing activated (65% eno1, 35% lan0)"
        fi
        ...
        if [ ! -f /tmp/isp2_down ]; then
            ...
            log_message "ISP2 (lan0) down - Using ISP1 (eno1) only"
        fi
        ...
        if [ ! -f /tmp/isp1_down ]; then
            ...
            log_message "ISP1 (eno1) down - Using ISP2 (lan0) only"
        fi
    else
        if [ ! -f /tmp/both_down ]; then
            touch /tmp/both_down
    ...
    # Update detailed status
    {
        ...
        echo "Active Mode:"
        if [ -f /tmp/load_balance_active ]; then
            echo "⚖️  Load Balancing (65% eno1, 35% lan0)"
        elif [ -f /tmp/isp2_down ]; then
            echo "🔵 Single ISP - eno1 only"
        elif [ -f /tmp/isp1_down ]; then
            echo "🟢 Single ISP - lan0 only"
        else
            echo "🔴 Both ISPs Down"
        fi
        echo
